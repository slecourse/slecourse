...
parseBlockStatements
  = do vds <- many parseVariableDeclaration
       ss  <- many parseStatement
       return (BlockStatements vds ss)
...
parseStatementNoBlock
  = symbols [(";",Skip)]
    ...
    +++
    fmap StatFocus ( bracket (parseSymbol "<<")
                             parseStatement
                             (parseSymbol ">>") )
...
parseClassDeclaration
  = do parseSymbol "public"
       ...
       mds <- many parseMethodDeclaration
       parseSymbol "}"
...
parseFormalParameters
  = fmap FormalParams (sepby parseFormalParameter (parseSymbol ","))
